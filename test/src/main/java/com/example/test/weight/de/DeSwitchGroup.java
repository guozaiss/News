...
public class DeSwitchGroup extends ViewGroup {
    ...
    public DeSwitchGroup(Context context, AttributeSet attrs) {
        super(context, attrs);


        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.DeSwitchGroup);
        mPinnedViewResId = a.getResourceId(R.styleable.DeSwitchGroup_pinned_view1, 0);
        mOrientation = a.getInt(R.styleable.DeSwitchGroup_orientation1, 0);
        int itemResId = a.getResourceId(R.styleable.DeSwitchGroup_pinned_item1, 0);
        int itemArrayResId = a.getResourceId(R.styleable.DeSwitchGroup_pinned_item_array1, 0);
        ...
        if (itemResId != 0 && itemArrayResId != 0) {
            ...
            for (int i = 0; i < size; i++) {
                DeSwitchItemView switchItemView = (DeSwitchItemView) LayoutInflater.from(getContext()).inflate(itemResId, null);
                ...
                if (event.getAction() == MotionEvent.ACTION_UP || event.getAction() == MotionEvent.ACTION_CANCEL) {
                    ...
                    if (frame.contains(scrolledXInt, scrolledYInt)) {
                        if (!child.isSelected()) {
                            ...
                            if (child instanceof DeSwitchItemView) {
                                ((DeSwitchItemView) child).handlerPinnedView(mPinnedView);
                                if (mItemHander != null)
