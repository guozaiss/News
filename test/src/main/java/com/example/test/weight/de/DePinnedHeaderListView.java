 ...
 */
public class DePinnedHeaderListView extends ListView implements AbsListView.OnScrollListener {


    private static final int MAX_ALPHA = 255;

    private DePinnedHeaderAdapter mAdapter;
    ...
    public DePinnedHeaderListView(Context context) {
        super(context);
    }

    public DePinnedHeaderListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public DePinnedHeaderListView(Context context, AttributeSet attrs, int defStyle) {
...
//        a.recycle();
    ...
    public void setAdapter(ListAdapter adapter) {
        ...
        if (!(adapter instanceof DePinnedHeaderAdapter)) {
            throw new RuntimeException("Adapter not instance of PinnedHeader");
        }
        mCurrentHeader = null;
        super.setAdapter(adapter);
        mAdapter = (DePinnedHeaderAdapter) adapter;
    }
...
//        }
        ...
        switch (state) {
            case DePinnedHeaderAdapter.PINNED_HEADER_GONE: {
                mHeaderViewVisible = false;
                break;
            }

            case DePinnedHeaderAdapter.PINNED_HEADER_VISIBLE: {
            ...
            case DePinnedHeaderAdapter.PINNED_HEADER_PUSHED_UP: {
