...
public class HelloChartsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Line line = new Line(values).setColor(Color.BLUE).setHasPoints(false).setCubic(true);
        Line line2 = new Line(values2).setColor(Color.RED).setHasPoints(false).setCubic(true);
        Line line3 = new Line(values3).setColor(Color.GREEN).setHasPoints(false).setCubic(true);
        ...
        data.setLines(lines);
        data.setAxisXBottom(new Axis(axisValues).setHasLines(true).setTextColor(color).setLineColor(color));
        data.setAxisYLeft(new Axis(axisValues).setHasLines(true).setTextColor(color).setLineColor(color).setMaxLabelChars(3));
